...
PROCESS(broadcast_response_process, "Broadcast response process");
// PROCESS(runicast_process, "Runicast process");
AUTOSTART_PROCESSES(&broadcast_process, &broadcast_response_process/*, &runicast_process*/);
/*---------------------------------------------------------------------------*/
...
broadcast_recv(struct broadcast_conn *c, const linkaddr_t *from)
{
  ...
  while(runicast_is_transmitting(&runicast_broadcast)) {
    printf("loop runicast_broadcast\n");
  }
  packetbuf_clear();
...
PROCESS_THREAD(broadcast_response_process, ev, data)
{
  PROCESS_EXITHANDLER(runicast_close(&runicast_broadcast);)
